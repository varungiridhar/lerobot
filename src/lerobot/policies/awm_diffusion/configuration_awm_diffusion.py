...
@PreTrainedConfig.register_subclass("awm_diffusion")
@dataclass
class AWMDiffusionConfig(PreTrainedConfig):
    ...
    def __post_init__(self):
        ...
        if self.wm_diffusion_timesteps <= 0:
            raise ValueError(
                f"`wm_diffusion_timesteps` must be > 0. Got {self.wm_diffusion_timesteps}."
            )
        if self.wm_diffusion_beta_start <= 0 or self.wm_diffusion_beta_end <= 0:
            raise ValueError(
                f"Diffusion betas must be positive. Got start={self.wm_diffusion_beta_start}, "
                f"end={self.wm_diffusion_beta_end}."
            )
        if self.wm_diffusion_beta_start >= self.wm_diffusion_beta_end:
            raise ValueError(
