...
from lerobot.policies.awm_diffusion.configuration_awm_diffusion import AWMDiffusionConfig as AWMConfig
from lerobot.policies.awm_diffusion.tokenizer_awm_diffusion import UniformActionTokenizer
...
def _extract_per_batch(buffer: Tensor, t: Tensor) -> Tensor:
    ...
    #   buffer: (n_timesteps,)
    #   t:      (B,)
    # returns: (1, B, 1) for broadcasting against (S, B, D)
    return buffer.gather(0, t).view(1, -1, 1)
...
class SinusoidalTimeEmbedding(nn.Module):
    ...
    def forward(self, t: Tensor) -> Tensor:
        ...
        half = self.dim // 2
        freq = torch.exp(
            torch.arange(half, device=device, dtype=torch.float32) * (-torch.log(torch.tensor(10000.0, device=device)) / max(half - 1, 1))
        )
...
class AWMDiffusionPolicy(PreTrainedPolicy):
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        z_pred, z_target, decoded_curr, gt_curr_img, wm_diffusion_loss_per_sample = wm_tensors
        valid_wm = ~next_obs_is_pad[:, 1]  # (B,)
        wm_diffusion_loss = (wm_diffusion_loss_per_sample * valid_wm).sum() / valid_wm.sum().clamp(min=1)
        cos_sim = F.cosine_similarity(z_pred, z_target, dim=-1).mean(dim=0)  # mean over S → (B,)
        wm_cosine = (cos_sim * valid_wm).sum() / valid_wm.sum().clamp(min=1)

        weighted_wm_loss = self.config.wm_loss_weight * wm_diffusion_loss
        loss = action_loss + weighted_wm_loss
        info = {
            "action_loss": action_loss.item(),
            "wm_diffusion_loss": wm_diffusion_loss.item(),
            "wm_loss": wm_diffusion_loss.item(),  # Backward-compatible alias.
            "wm_loss_weighted": weighted_wm_loss.item(),
            "wm_cosine": wm_cosine.item(),
        }
    ...
    def visualize(self, batch: dict[str, Tensor], n_pairs: int = 12) -> dict[str, Tensor] | None:
        ...
        # Run WM diffusion to get future state prediction.
        token_ids = self.model.tokenizer.encode(batch[ACTION][:n])
        z_pred = self.model.sample_wm_latents(token_ids, batch_size, curr_encoder_in, cross_pos)
...
class AWM(nn.Module):
    ...
    def __init__(self, config: AWMConfig):
        ...
        betas = torch.linspace(
            config.wm_diffusion_beta_start,
            config.wm_diffusion_beta_end,
            config.wm_diffusion_timesteps,
        )
        alphas = 1.0 - betas
        ...
        self.register_buffer("diff_alphas_cumprod", alphas_cumprod)
        self.register_buffer("diff_sqrt_alphas_cumprod", torch.sqrt(alphas_cumprod))
        self.register_buffer("diff_sqrt_one_minus_alphas_cumprod", torch.sqrt(1.0 - alphas_cumprod))
        self.register_buffer("diff_sqrt_recip_alphas", torch.sqrt(1.0 / alphas))
        self.register_buffer(
            "diff_posterior_variance",
    ...
    def _build_wm_context(self, token_ids: Tensor, batch_size: int, encoder_in: Tensor, cross_pos: Tensor) -> Tensor:
        """Build WM conditioning tokens in latent space (S, B, dim_model)."""
        T = token_ids.shape[1]
    ...
    def _q_sample(self, z0: Tensor, t: Tensor, noise: Tensor) -> Tensor:
        sqrt_alpha_bar_t = _extract_per_batch(self.diff_sqrt_alphas_cumprod, t)
    ...
    def _predict_z0(self, z_t: Tensor, t: Tensor, noise_pred: Tensor) -> Tensor:
        sqrt_alpha_bar_t = _extract_per_batch(self.diff_sqrt_alphas_cumprod, t)
        sqrt_one_minus_alpha_bar_t = _extract_per_batch(self.diff_sqrt_one_minus_alphas_cumprod, t)
        return (z_t - sqrt_one_minus_alpha_bar_t * noise_pred) / sqrt_alpha_bar_t.clamp(min=1e-6)

    @torch.no_grad()
    def sample_wm_latents(self, token_ids: Tensor, batch_size: int, encoder_in: Tensor, cross_pos: Tensor) -> Tensor:
        """Sample predicted future latent tokens z_pred via DDPM reverse process."""
        wm_cond = self._build_wm_context(token_ids, batch_size, encoder_in, cross_pos)
        z_t = torch.randn_like(wm_cond)
        ...
        for step in reversed(range(self.n_diffusion_steps)):
            ...
            sqrt_one_minus_alpha_bar_t = _extract_per_batch(self.diff_sqrt_one_minus_alphas_cumprod, t)
            sqrt_recip_alpha_t = _extract_per_batch(self.diff_sqrt_recip_alphas, t)
            model_mean = sqrt_recip_alpha_t * (z_t - (beta_t / sqrt_one_minus_alpha_bar_t.clamp(min=1e-6)) * noise_pred)

            if step > 0:
                posterior_var_t = _extract_per_batch(self.diff_posterior_variance, t)
                noise = torch.randn_like(z_t)
                z_t = model_mean + torch.sqrt(posterior_var_t.clamp(min=1e-12)) * noise
            else:
                z_t = model_mean
    ...
    def forward(
        ...
        Returns:
            ...
            wm_tensors: ``(z_pred, z_target, decoded_curr, gt_curr_img, wm_diffusion_loss_per_sample)``.
        """
