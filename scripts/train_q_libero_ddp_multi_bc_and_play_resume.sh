...
# Resume the multi-dataset Q-function DDP run (BC + 4 LIBERO play splits)
# from its last checkpoint. Same GPU layout as
# scripts/train_q_libero_ddp_multi_bc_and_play.sh.
#
...
#     continues in the SAME output_dir (new checkpoints append: 007000, ...).
...
#     mkdir -p logs && sbatch scripts/train_q_libero_ddp_multi_bc_and_play_resume.sh
#
...
#     CKPT_DIR=outputs/train/.../<run_dir> sbatch scripts/train_q_libero_ddp_multi_bc_and_play_resume.sh
...
CKPT_DIR="${CKPT_DIR:-outputs/train/2026-05-19/23-54-24_qf_libero_ddp2_bsz48_bc_plus_play_h200}"
# Resume from the latest good checkpoint of the previous run (step 18000). Bump
# CKPT_STEP (or set CKPT_STEP=last) when resuming again from a later checkpoint.
CKPT_STEP="${CKPT_STEP:-018000}"
